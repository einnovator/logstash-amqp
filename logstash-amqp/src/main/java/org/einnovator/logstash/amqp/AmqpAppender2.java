 ...
 * 
 */
package org.einnovator.logstash.amqp;
 ...
 * @since 1.4
 */
public class AmqpAppender2 extends AppenderBase<ILoggingEvent> {
	...
	protected class EventSender implements Runnable {
		...
		public void run() {
			try {
				RabbitTemplate rabbitTemplate = new RabbitTemplate(AmqpAppender2.this.connectionFactory);
				while (true) {
					final Event event = AmqpAppender2.this.events.take();

					MessageProperties amqpProps = prepareMessageProperties(event);

					String routingKey = AmqpAppender2.this.routingKeyLayout.doLayout(event.getEvent());

					sendOneEncoderPatternMessage(rabbitTemplate, routingKey);
					
					System.out.println(this + ": #" + AmqpAppender2.this.events.size());
		...
		private void sendOneEncoderPatternMessage(RabbitTemplate rabbitTemplate, String routingKey) {
			 ...
			 */
			if (AmqpAppender2.this.encoder != null
					&& AmqpAppender2.this.headerWritten.compareAndSet(false, true)) {
				byte[] header = AmqpAppender2.this.encoder.headerBytes();
				if (header != null && header.length > 0) {
					rabbitTemplate.convertAndSend(AmqpAppender2.this.exchangeName, routingKey, header, m -> {
						if (AmqpAppender2.this.applicationId != null) {
							m.getMessageProperties().setAppId(AmqpAppender2.this.applicationId);
						}
						return m;
					});
				}
		...
		private void doSend(RabbitTemplate rabbitTemplate, final Event event, ILoggingEvent logEvent, String name,
			...
			byte[] msgBody;
			if (AmqpAppender2.this.abbreviator != null && logEvent instanceof LoggingEvent) {
				((LoggingEvent) logEvent).setLoggerName(AmqpAppender2.this.abbreviator.abbreviate(name));
			...
			try {
				...
				rabbitTemplate.send(AmqpAppender2.this.exchangeName, routingKey, message);
			}
			catch (AmqpException e) {
				int retries = event.incrementRetries();
				if (retries < AmqpAppender2.this.maxSenderRetries) {
					// Schedule a retry based on the number of times I've tried to re-send this
					AmqpAppender2.this.retryTimer.schedule(new TimerTask() {

						@Override
						public void run() {
							AmqpAppender2.this.events.add(event);
						}
					...
					addError("Could not send log message " + logEvent.getMessage()
							+ " after " + AmqpAppender2.this.maxSenderRetries + " retries", e);
				}
			}
		}
		...
		private byte[] encodeMessage(ILoggingEvent logEvent) {
			if (AmqpAppender2.this.encoder != null) {
				return AmqpAppender2.this.encoder.encode(logEvent);
			}

			String msgBody = AmqpAppender2.this.layout.doLayout(logEvent);
			if (AmqpAppender2.this.charset != null) {
				try {
					return msgBody.getBytes(AmqpAppender2.this.charset);
				}
